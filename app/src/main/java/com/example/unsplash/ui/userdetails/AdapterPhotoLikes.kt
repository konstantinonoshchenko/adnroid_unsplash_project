...
import com.example.unsplash.databinding.ItemUnsplashPhotoWidthFullScreenBinding
...
class AdapterPhotoLikes(
    ...
    override fun onBindViewHolder(holder: MySimpleViewHolder, position: Int) {
        ...
        holder.binding.apply {
            ...
            val download = holder.binding.root.context.getString(R.string.download)
        ...
        holder.binding.likesWidthFullScreen.setOnClickListener { _ ->
            item?.let { it -> onClickLike(it.id.toString(), position) }
        }

        holder.binding.downloadWidthFullScreen.setOnClickListener { _ ->
            item!!.urls?.raw?.let { item.id?.let { id -> onClickDownload(id, it,holder.binding.downloadWidthFullScreen) } }
        }
